...
inline fun <reified T> Context.getPrefVal(key: PrefKey, default: T? = null): T {
    return getSharedPreferences("aac_pref", Context.MODE_PRIVATE).let {
        when (T::class) {
            String::class ->
                it.getString(key.name.toLowerCase(), default as? String? ?: "") as T
            Int::class ->
                it.getInt(key.name.toLowerCase(), default as? Int? ?: 0) as T
            Long::class ->
                it.getLong(key.name.toLowerCase(), default as? Long? ?: 0L) as T
            Boolean::class ->
                it.getBoolean(key.name.toLowerCase(), default as? Boolean? ?: false) as T
            Float::class ->
                it.getFloat(key.name.toLowerCase(), default as? Float? ?: 0F) as T
            else ->
                throw IllegalStateException("not support class[${T::class}]")
        }
    }
}
